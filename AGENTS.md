...
│   ├── dappStyle.ts              # Per-dapp accent + emoji identity (DappAvatar)
...
│   │   ├── RestaurantApp.tsx     # Tabbed menu mini-app: Order / Rewards / History + pickup QR
...
| `homeShortcuts` | Customizable Home tile grid (persisted); `addShortcut` / `removeShortcut` / `resetShortcuts`; `DEFAULT_SHORTCUTS` |
| `orders` | Menu/restaurant orders (persisted); `placeOrder(o)→record` for the History tab + pickup QR; `addPoints(ens, n)` for points-only loyalty |
...
|---|---|---|---|
| `EXPO_PUBLIC_ANTHROPIC_API_KEY` | `agent.ts` | Claude tool-calling agent (direct Messages API) | Falls back to `EXPO_PUBLIC_ANTHROPIC_PROXY_URL` (Claude Code subscription proxy), then `assistant.ts` template generator |
...
**`menu` (restaurant ordering):** `{ currency, items: [{ id, name, priceUsd, desc?, tag? }], pointsPerDollar? }`. Any `menu` dapp renders the **`RestaurantApp`** tabbed mini-app (not the generic runtime): **Order** (cart via `MenuOrder` → pay total through `runFlow`, earn `pointsPerDollar` points), **Rewards** (redeem `POINTS_REWARDS` via `spendPoints`, shows the earn rate), **History** (past `orders`). It's **points-only — no `punchCard`/stamps**; points accrue via `addPoints`. After paying, a confirmation shows a **pickup QR** (`dappdock://order/<id>`) with the buyer's ENS/short-address + unique order id so the merchant can verify the order. Cart is runtime state, not a manifest field.
...
| Agent says “template mode” | No Anthropic key or proxy | Set `EXPO_PUBLIC_ANTHROPIC_API_KEY`, or run `dappdock-claude-proxy` and set `EXPO_PUBLIC_ANTHROPIC_PROXY_URL` to your Mac LAN IP, restart with `-c` |
...
| Date | Author | Change |
|---|---|---|
| 2026-06-13 | Review+motion agent | **Audit (5 overlapping subagents) + motion polish.** Ran two crash/correctness auditors, two design/motion auditors, and one sponsor-integration auditor in parallel. No high-severity crashers found; fixed the real findings: (1) **on-chain loyalty merge** — `readLoyalty` now shape-validates each record (drops malformed values that made `addStamp` → NaN) and `syncLoyaltyFromChain` reconciles **field-wise max** so the ENS read-through never clobbers local gains; (2) runtime "Done." off-by-one stamp count (uses post-increment); (3) `validateManifest` now rejects negative/NaN menu `priceUsd` + non-negative `pointsPerDollar`; (4) `parseUnits(amt.toFixed(6),6)` in wallet/execution/composer so free-text amounts can't throw "too many decimals"; (5) re-applied World ID fixes that had reverted — `BRIDGE = ENV.worldBridgeUrl`, request `verification_level = ENV.worldVerificationLevel`, and `hasWorldCreds` now accepts an `rp_id`; (6) red-packet reveal copy made honest (recorded, not "settling via LI.FI"); (7) `PunchCard` stamp glyph is now the dapp's own `dappEmoji(ens)` not a hardcoded 🍔. **Motion:** new `ui.tsx` primitives `usePressScale`/`PressableScale` (press feedback now on `PrimaryButton`/`Chip`/`ListRow`/`SearchPill`), `CountUp` (wallet balance + rewards points tick up), `SuccessCheck` (spring-pop on runtime done + red-packet reveal); `PunchCard` newest stamp "lands" with a spring; Activity list cascades in (staggered `FadeUp`). `tsc` + iOS bundle (4762 modules) clean. |
| 2026-06-13 | Build agent | **Claude Code proxy fallback for design agent.** Wired `EXPO_PUBLIC_ANTHROPIC_PROXY_URL` (+ optional `_PROXY_KEY`) in `env.ts` / `agent.ts`: direct `EXPO_PUBLIC_ANTHROPIC_API_KEY` still wins; when unset the app calls the local FastAPI proxy at `~/Documents/GitHub/dappdock-claude-proxy` (Anthropic-compatible `/v1/messages` via Claude Agent SDK + subscription). Assistant header shows “Claude Code proxy” vs “template mode”. `.env` configured with LAN IP `192.168.14.95:3456`; proxy verified live (`proxy-ok` + `tool_use`). `tsc` clean. |
| 2026-06-13 | Review agent | **Bug-fix pass (review → fix).** Five confirmed bugs fixed after a full `tsc` + iOS `expo export` + lint review (all already green; these were logic/flow bugs). (1) **Pickup-order QR was unscannable** — `RestaurantApp` emits `dappdock://order/<id>` but `links.ts` `routeForPayload` didn't know that scheme, so Scan showed "doesn't contain a dapp link"; it now reopens the order's dapp (unknown id → `/activity`). (2) **`/runtime/draft` with a cleared draft** silently ran the first seed dapp; now `<Redirect>`s to `/assistant` like preview/publish. (3) **Publish builder-credits label** showed `4 - builderCredits` ("1 of 3" before any publish); now shows remaining `${builderCredits} of 3 … left` (matches Profile's `/3`). (4) **Publish ignored the 3-credit cap** — `publish()` decremented into negatives; now blocked + CTA disabled at `builderCredits <= 0`. (5) **Edited memo was discarded** — runtime now carries the (possibly edited) memo onto the receipt and `/activity` renders `note`. `tsc` + iOS bundle + lint clean. |
...
*Last reviewed against codebase: 2026-06-13 (SDK 54, native-only; pure-viem ENS + ENSIP-26, World ID 4.0 verify + AgentKit free-trial, LI.FI Composer; Node services in `server/`; customizable Home, RestaurantApp mini-app, persistent tab bar + theme crossfade).*
